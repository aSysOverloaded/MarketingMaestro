...
def test_repeated_page_furniture_is_detected_and_stripped():
    boilerplate = search.find_boilerplate_lines(_pages())
    assert NAV in boilerplate
    assert "Product 0 details" not in boilerplate  # per-page content survives

    cleaned = search.strip_boilerplate(_pages()[0], boilerplate)
    assert cleaned.splitlines()[0] == "Product 0 details"


def test_boilerplate_needs_a_real_repeat_and_short_documents_are_left_alone():
    assert search.find_boilerplate_lines(["a", "b"]) == set()  # too few pages to judge
    pages = _pages(10)
    pages[0] += "\nAppears twice only"
    pages[1] += "\nAppears twice only"
    assert "Appears twice only" not in search.find_boilerplate_lines(pages)
...
def test_hero_image_is_the_largest_on_the_page_not_the_first(tmp_path):
    ...
    paths = search.select_page_images(page, 54, str(tmp_path))

    assert len(paths) == search.MAX_IMAGES_PER_PAGE
    ...
    assert len(list(tmp_path.iterdir())) == search.MAX_IMAGES_PER_PAGE  # only winners written
...
def test_undecodable_images_fall_back_to_byte_size(tmp_path):
    ...
    paths = search.select_page_images(page, 1, str(tmp_path))
